...
class AddOptionDialog(QDialog):
    def __init__(self, db: Database, parent=None):
        ...
        for q in questions:
            ...
            q_lbl.setStyleSheet("font-weight: bold; margin-top: 10px; color: #e94560;")
            self.rules_layout.addWidget(q_lbl)
            
            answers = self._db.get_question_answers(q.id)
            for a in answers:
                if a.value == "dk": continue
                
                row = QHBoxLayout()
                row.addWidget(QLabel(f"  • {a.label}"))
                
                btn_perfect = QPushButton("Ідеально")
                btn_good = QPushButton("Добре")
                btn_neutral = QPushButton("Байдуже")
                btn_bad = QPushButton("Погано")
                btn_critical = QPushButton("Недоречно")
                
                btn_perfect.setStyleSheet("background-color: #4CAF50; color: white;")
                btn_good.setStyleSheet("background-color: #2196F3; color: white;")
                btn_neutral.setStyleSheet("background-color: #9E9E9E; color: white;")
                btn_bad.setStyleSheet("background-color: #FF9800; color: white;")
                btn_critical.setStyleSheet("background-color: #F44336; color: white;")
                ...
                btn_perfect.clicked.connect(lambda checked, s=spin: s.setValue(15))
                btn_good.clicked.connect(lambda checked, s=spin: s.setValue(5))
                btn_neutral.clicked.connect(lambda checked, s=spin: s.setValue(0))
                btn_bad.clicked.connect(lambda checked, s=spin: s.setValue(-5))
                btn_critical.clicked.connect(lambda checked, s=spin: s.setValue(-1000))
                
                row.addWidget(btn_perfect)
                row.addWidget(btn_good)
                row.addWidget(btn_neutral)
                row.addWidget(btn_bad)
                row.addWidget(btn_critical)
                row.addWidget(spin)
                
                self.rules_layout.addLayout(row)
                self.spinboxes.append((q.id, a.value, spin))
    ...
    def __init__(self, db: Database, option_id: int, option_text: str, parent=None):
        ...
        for q in questions:
            ...
            for a in answers:
                ...
                curr_val = current_rules.get((q.id, a.value), 0.0)
...
class AddQuestionDialog(QDialog):
    ...
    def _add_answer_row(self):
        ...
        lbl_rules = QLabel(f"Бали для варіанту '{val}':")
        lbl_rules.setStyleSheet("font-weight: bold; margin-top: 10px; color: #e94560;")
        self.rules_layout.addWidget(lbl_rules)
        ...
        for opt in self.options:
            ...
            row_layout.addWidget(btn_perfect)
            row_layout.addWidget(btn_good)
            row_layout.addWidget(btn_neutral)
            row_layout.addWidget(btn_bad)
            row_layout.addWidget(btn_critical)
            row_layout.addWidget(spin)
            
            self.rules_layout.addLayout(row_layout)
            self.rules_widgets.append((val, opt.id, spin))
...
class EditQuestionDialog(QDialog):
    ...
    def _add_answer_row_with_data(self, val: str, label: str):
        ...
        for opt in self.options:
            ...
            curr_val = current_ans_rules.get(opt.id, 0.0)
            spin.setValue(curr_val)
